...
class TokenTaggingDataset(Dataset):
    def __init__(
        ...
        self.freq_N = dict([(i+1, 0) for i in range(self.max_segment_len)])
        self.reached_max_freq = dict([(i+1, False) for i in range(self.max_segment_len)])
        self.max_freq = 3     
    ...
    def __getitem__(self, idx):
        ...
        #N = None 
        #for n in range(self.max_segment_len, 0, -1):
        #    if not self.reached_max_freq[n] and seqlen >= n:
        #        N = n 
        #        break
        #if N is None:
        #    N = np.random.choice(np.arange(seqlen)) + 1

        #self.freq_N[N] += 1
        #if self.freq_N[N] == self.max_freq:
        #    self.reached_max_freq[N] = True

        #pdb.set_trace()
        n = np.argmin(self.segment_freqs[:seqlen])
        N = n + 1
        self.segment_freqs[n] += 1
        #print(self.segment_freqs, N, seqlen)

        #N = ((self.max_segment_len - 1) - np.argmin(self.segment_freqs[-seqlen:])) + 1
        #self.segment_freqs[self.max_segment_len-N] += 1

        #pdb.set_trace()
        segment_num = np.random.choice(np.arange((len(labeled_indices) - N) + 1))
        start_index = segment_num + 1
        ...
        for key in ["tokenized_text", "tokenized_labels", "input_ids", "loss_mask"]:
            item[f"original_{key}"] = item[key]
            item[key] = [item[key][0]] + item[key][start_index:end_index+1] + [item[key][-1]]
        item["length"] = N
        #self.batch_count += 1

        #if False not in list(self.reached_max_freq.values()):
        #    self.freq_N = dict([(i+1, 0) for i in range(self.max_segment_len)])
        #    self.reached_max_freq = dict([(i+1, False) for i in range(self.max_segment_len)])
...
class TokenTaggingDataset_train(Dataset):
    def __init__(
        self,
        input_texts,
        targets,
        ...
        cnt_failed = 0
        # Perform preprocessing at initialization
        temp_preprocessed_data = dict()
        self.window_length_counts = dict()
        for i in range(10):
            temp_preprocessed_data[i+1] = []
            self.window_length_counts[i+1] = 0
        self.processed_data = []
        for text, labels_per_word in tqdm(
            zip(self.inputs, self.targets),
            total=len(self.inputs),
            ...
            for context_length in range(1,11):
                if context_length <= seqlen:
                    for meta_index, start_index in enumerate(labeled_indices[:(seqlen-context_length)+1]):
                        ...
                        temp_preprocessed_data[context_length].append(item)
                        self.window_length_counts[context_length] += 1
        
        min_count = min(self.window_length_counts.values())
        for i in range(10):
            indices = np.random.choice(self.window_length_counts[i+1], min_count, replace=False)
            self.processed_data.extend([temp_preprocessed_data[i+1][k] for k in indices])
...
class TokenTaggingDataset_test(Dataset):
    def __init__(
        self,
        input_texts,
        targets,
